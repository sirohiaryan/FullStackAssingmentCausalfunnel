  ...
  if (loading) {
    return (
      <div class="flex flex-col items-center justify-center py-24 space-y-3">
        ...
        <p class="text-xs font-mono text-zinc-500">streaming_active_telemetry...</p>
      </div>
    );
  }
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
      ...
      <div class="lg:col-span-5 bg-[#09090b] rounded-xl border border-zinc-800/80 overflow-hidden shadow-2xl">
        <div class="p-4 bg-[#141416]/60 border-b border-zinc-800/80 flex justify-between items-center">
          <h2 class="text-xs font-mono font-semibold text-zinc-400 uppercase tracking-wider">Identified Sessions</h2>
        ...
        <div class="divide-y divide-zinc-900 max-h-[72vh] overflow-y-auto">
          {sessions.map(sess => (
            <div 
              key={sess._id} 
              onClick={() => loadStream(sess._id)}
              class={`p-4 cursor-pointer transition-all duration-150 relative ${selected === sess._id ? 'bg-[#141416] border-l-2 border-white' : 'hover:bg-[#141416]/40'}`}
            >
              <div class="flex justify-between items-center mb-2">
                <span class="font-mono text-xs font-medium text-white tracking-tight">{sess._id}</span>
      ...
      {/* Right Column: Telemetry Journey Log */}
        ...
        {selected ? (
          <>
            <div class="mb-5 pb-4 border-b border-zinc-800 flex flex-col gap-1">
              <h3 class="text-xs font-mono font-semibold text-zinc-400 uppercase tracking-wider">User Journey Stream Pipeline</h3>
              <p class="text-[11px] text-zinc-500 font-mono">ID: <span class="text-zinc-300">{selected}</span></p>
            </div>
                  ...
                  <div class="flex-1 min-w-0 space-y-2">
                    ...
                    {event.eventType === 'click' && (
                      <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] font-mono text-zinc-500 bg-[#09090b]/40 p-2 rounded border border-zinc-900/60">
                        <span>matrix_x: <strong class="text-zinc-300 font-normal">{event.clickX}%</strong></span>
                        <span>matrix_y: <strong class="text-zinc-300 font-normal">{event.clickY}%</strong></span>
